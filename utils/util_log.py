...
class TestLogConfig:
    ...
    def set_log_level(self, use_stream=True):
        try:
            _format = "[%(asctime)s - %(levelname)5s - %(name)s]: %(message)s (%(fn)s:%(ln)s)"
...
class TestLog:
    ...
    def debug(self, msg, *args, **kwargs):
        ...
        self.test_log.debug(msg, extra={"fn": str(sys._getframe().f_back.f_code.co_filename).split('/')[-1],
                                        "ln": sys._getframe().f_back.f_lineno}, *args, **kwargs)

    def info(self, msg, *args, **kwargs):
        self.test_log.info(msg, extra={"fn": str(sys._getframe().f_back.f_code.co_filename).split('/')[-1],
                                       "ln": sys._getframe().f_back.f_lineno}, *args, **kwargs)

    def warning(self, msg, *args, **kwargs):
        self.test_log.warning(msg, extra={"fn": str(sys._getframe().f_back.f_code.co_filename).split('/')[-1],
                                          "ln": sys._getframe().f_back.f_lineno}, *args, **kwargs)

    def error(self, msg, *args, **kwargs):
        self.test_log.error(msg, extra={"fn": str(sys._getframe().f_back.f_code.co_filename).split('/')[-1],
                                        "ln": sys._getframe().f_back.f_lineno}, *args, **kwargs)
